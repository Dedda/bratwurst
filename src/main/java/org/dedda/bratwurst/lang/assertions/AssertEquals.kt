 ...
 * @author dedda
 */
class AssertEquals(lineNumber: Int, private val variableNameLeft: String, private val variableNameRight: String) :
    ...
    override fun doAssertion(scope: Scope): AssertionResult {
        ...
        if (left.javaClass != right.javaClass) {
            ...
            if (!equal) {
                when (left) {
                    is BWInteger -> {
                        message = "Integers ($left) and ($right) not equal!"
                    }
                    is BWString -> {
                        message = ("Strings (${left.stringValue}) and (${(right as BWString).stringValue}) not equal!")
                    }
                    is BWObject -> {
